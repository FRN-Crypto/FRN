...
*Setup instructions for a Gitian build of Francs using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Francs
...
- [Building Francs](#building-Francs)
...
Clone the git repositories for Francs and Gitian and then checkout the Francs version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Francs-project/Francs.git
cd Francs
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Francs repository
...
To build Francs (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the Francs repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Francs/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Francs-project/Francs
...
For example:
```bash
URL=https://github.com/user/Francs.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Francs=${COMMIT} --url Francs=${URL} ../Francs/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Francs=${COMMIT} --url Francs=${URL} ../Francs/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Francs=${COMMIT} --url Francs=${URL} ../Francs/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Francs-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Francs-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Francs-build.assert
```
...
[Francs/gitian.sigs](https://github.com/Francs-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
