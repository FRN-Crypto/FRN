...
// Copyright (c) 2011-2012 Francs Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FrancsTEST0\r");
            Send(hSocket, "WHO #FrancsTEST0\r");
        } else {
            // randomly join #Francs00-#Francs99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Francs%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Francs%02d\r", channel_number).c_str());
        }
