...
uint256 hashGenesisBlock("0xbd01d5bff63876c9d5f2bc7a3e36fc7d3685b899f18c44fec09f8cf4835f839f");
...
const string strMessageMagic = "Francs Signed Message:\n";
...
{
    // Francs: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Francs: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Francs Genesis Block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1446667373;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 302581;

        if (fTestNet)
        {
        	block.nTime    = 1446667072;
            block.nNonce   = 1084764;
        }
		...
		if(!fTestNet)
		{
			assert(block.hashMerkleRoot == uint256("0x4951c14dd98b7acbcc68584f8cfb3da32a80861b328a342772740f01ddd1c13b"));
			block.print();
...
unsigned char pchMessageStart[4] = { 0xee, 0xea, 0xae, 0xef };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FrancsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrancsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FrancsMiner(CWallet *pwallet)
{
    printf("FrancsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Francs-miner");
    ...
    try { loop {
        ...
        printf("Running FrancsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FrancsMiner, pwallet));
}
