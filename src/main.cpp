...
#if defined(NDEBUG)
# error "Francs cannot be compiled without assertions."
#endif
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if ((!Params().SkipProofOfWorkCheck()) &&
       (block.nBits != GetNextWorkRequired(pindexPrev, &block)))
        return state.DoS(100, error("%s : incorrect proof of work", __func__, nHeight),
    ...
    // Francs: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
    bool enforceV2 = false;
    if (block.nVersion < 2)
    {
        if (Params().EnforceV2AfterHeight() != -1)
        {
            // Mainnet 999m, Testnet 400k
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Francs: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
